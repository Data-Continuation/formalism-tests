...
WORK_DIR = Path(".tmp/stegverse-001-remote-core-lite")
...
class ReceiptChain:
    ...
    def record(self, event_type: str, decision: str, basis: str, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
        receipt = {
            "schema": "stegverse_001_remote_operator_receipt.v1",
...
def parse_python(path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    text = read_text(path)
    contract: dict[str, Any] = {"path": rel, "exists": path.exists() and path.is_file(), "sha256": sha256_file(path), "functions": [], "classes": [], "import_from": [], "imports": [], "parse_error": None}
    if not text:
        return contract
    try:
        tree = ast.parse(text)
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            contract["import_from"].append({"module": node.module or "", "names": [alias.name for alias in node.names]})
...
def inspect_task_manifests(repo_root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(task_dir.glob("*.json")):
        entry: dict[str, Any] = {"path": path.relative_to(repo_root).as_posix(), "sha256": sha256_file(path), "task_ids": [], "commands": [], "parse_error": None}
        try:
...
def clone_or_use_local(receipts: ReceiptChain) -> tuple[Path | None, dict[str, Any]]:
    ...
    if local:
        ...
        receipts.record("target_repo_selected", "LOCAL_PATH_SELECTED" if meta["success"] else "FAIL_CLOSED", "StegVerse-001 selected local target path.", meta)
    ...
    meta = {"mode": "clone", "repo": safe_repo, "branch": branch, "root": clone_dir.as_posix(), "success": proc.returncode == 0, "returncode": proc.returncode, "stdout_tail": proc.stdout[-2000:], "stderr_tail": proc.stderr[-2000:]}
    receipts.record("target_repo_cloned", "CLONED" if proc.returncode == 0 else "FAIL_CLOSED", "StegVerse-001 cloned target repo for inspection.", meta)
...
def determine_contract(repo_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    py_targets = {name: repo_root / "core_lite" / f"{name}.py" for name in ["cli", "cge", "ingest", "sandbox", "receipts"]}
    ...
    observed_cge_imports, missing_cge_exports = [], []
    for module_name, contract in py_contracts.items():
        for item in contract.get("import_from", []):
            if item.get("module") == "core_lite.cge":
                need = {"importing_module": module_name, "from_module": "core_lite.cge", "names": item.get("names", [])}
    ...
    surfaces = {
        "incoming": {"path": "incoming/", "exists": (repo_root / "incoming").exists(), "file_count": len([p for p in (repo_root / "incoming").rglob("*") if p.is_file()]) if (repo_root / "incoming").exists() else 0},
        "reports_current": {"path": "reports/current/", "exists": (repo_root / "reports" / "current").exists()},
        "receipts_current": {"path": "receipts/current/", "exists": (repo_root / "receipts" / "current").exists()},
        "workflows": workflows,
    ...
    if missing_cge_exports:
        blockers.append({"type": "cge_contractual_inclusion", "basis": "Target modules import symbols from core_lite.cge that are not exported by current cge.py.", "items": missing_cge_exports})
    for key, present in transition_requirements.items():
        if not present:
            blockers.append({"type": "missing_transition_surface", "basis": f"Required surface absent for active transition: {key}", "items": [key]})
    if missing_cge_exports:
        next_change = {"classification": "contractual_inclusion", "target": "core_lite/cge.py", "required_exports": sorted({item["name"] for item in missing_cge_exports}), "preserve_existing_exports": True, "basis": "Observed import contract requires these exports before existing intake/self-test surfaces can run."}
    elif blockers:
        next_change = {"classification": "minimal_surface_completion", "target": "first_missing_transition_surface", "basis": "Complete only the missing surface required by the active transition."}
    else:
        next_change = {"classification": "run_existing_intake", "target": "existing Core-Lite Intake workflow", "basis": "No structural blocker observed for the active transition."}
    report = {"schema": "stegverse_001_remote_core_lite_working_contract_report.v1", "generated_at": datetime.now(timezone.utc).isoformat(), "success": True, "actor": "StegVerse-001", "mode": "initialization", "target_repo_root": repo_root.resolve().as_posix(), "active_transition": "Core-Lite Recorded Ingestion + CGE + Sandbox Result Return", "python_contracts": py_contracts, "observed_cge_imports": observed_cge_imports, "cge_exports": sorted(cge_exports), "missing_cge_exports": missing_cge_exports, "surfaces": surfaces, "transition_requirements": transition_requirements, "blockers": blockers, "decision": "PLAN_RETURNED", "boundary": ["formalism-tests is the proof and command backdrop.", "core-lite is the remote target.", "This task does not patch, push, submit incoming bundles, or change workflows.", "STOP after report, plan, and receipt."]}
    plan = {"schema": "stegverse_001_remote_core_lite_remediation_plan.v1", "generated_at": report["generated_at"], "actor": "StegVerse-001", "transition": report["active_transition"], "decision": "RETURN_PLAN_ONLY", "install_authority": False, "production_authority": False, "workflow_change_authority": False, "push_authority": False, "incoming_submission_authority": False, "blockers": blockers, "next_admissible_change": next_change, "stop_condition": "Return plan and receipt. Do not mutate target repo."}
    return report, plan

def write_markdown(report: dict[str, Any], plan: dict[str, Any]) -> None:
    lines = ["# StegVerse-001 Remote Core-Lite Working Contract Report", "", "## Status", "", "```text", f"actor: {report['actor']}", f"mode: {report['mode']}", f"active_transition: {report['active_transition']}", f"decision: {report['decision']}", f"blocker_count: {len(report['blockers'])}", "```", "", "## Role Separation", "", "```text", "formalism-tests = proof and command backdrop", "core-lite = remote target", "StegVerse-001 = initialization-state remote operator", "```", "", "## Observed CGE Import Contract", ""]
    if report["observed_cge_imports"]:
        for item in report["observed_cge_imports"]:
            lines.append(f"- `{item['importing_module']}` imports `{', '.join(item['names'])}` from `{item['from_module']}`")
    else:
        lines.append("No direct `from core_lite.cge import ...` imports observed in inspected modules.")
    lines += ["", "## Missing CGE Exports", ""]
    ...
    lines += ["", "## Transition Requirements", "", "```text"]
...
def main() -> int:
    ...
    receipts.record("remote_operator_started", "RECEIVED", "StegVerse-001 remote operator started from formalism-tests backdrop.", {"target_repo": os.environ.get("CORE_LITE_REPO", DEFAULT_REPO), "branch": os.environ.get("CORE_LITE_BRANCH", DEFAULT_BRANCH)})
    target_root, clone_meta = clone_or_use_local(receipts)
    if target_root is None:
        failure = {"schema": "stegverse_001_remote_core_lite_working_contract_report.v1", "generated_at": datetime.now(timezone.utc).isoformat(), "success": False, "decision": "FAIL_CLOSED", "basis": "Could not access target repo.", "target_access": clone_meta}
        REPORT_JSON.write_text(json.dumps(failure, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        PLAN_JSON.write_text(json.dumps({"schema": "stegverse_001_remote_core_lite_remediation_plan.v1", "decision": "FAIL_CLOSED", "basis": "Could not access target repo."}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    receipts.record("remote_contract_determined", "PLAN_RETURNED", "StegVerse-001 determined remote core-lite working contract and returned a plan.", {"report": REPORT_JSON.as_posix(), "plan": PLAN_JSON.as_posix(), "blocker_count": len(report["blockers"]), "next_admissible_change": plan["next_admissible_change"]})
