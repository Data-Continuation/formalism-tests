#!/usr/bin/env python3
"""Validate the Morrison Runtime canonical-evidence gate fail-closed."""
...
def validate_canonical(record: dict) -> list[str]:
    errors: list[str] = []
    if record.get("schema") != "stegverse.morrison-runtime.canonical-execution-evidence.v1":
        errors.append("canonical.schema mismatch")
    if record.get("suite_id") != "morrison-runtime-commit-time-scope-v0.1":
    ...
    if not isinstance(task_results, dict) or set(task_results) != REQUIRED_TASKS:
        errors.append("canonical.task_results must contain exactly all three declared Morrison tasks")
