#!/usr/bin/env python3
"""Validate FI continuity interoperability canonical evidence fail-closed."""
...
PENDING = ROOT / "receipts/fi_transition_continuity_interop_canonical_execution_evidence.pending.json"
CANONICAL = ROOT / "receipts/fi_transition_continuity_interop_canonical_execution_evidence.json"
OUTPUT = ROOT / "reports/fi_transition_continuity_interop_canonical_evidence_gate.json"
...
REQUIRED_TASKS = {
    ...
    "check_fi_transition_continuity_interop_canonical_evidence_gate",
}
REQUIRED_HASHES = {
    "report_sha256",
...
def validate_pending(record: dict) -> list[str]:
    ...
    expected = {
        ...
        "downstream_activation_prohibited": True,
...
def validate_canonical(record: dict) -> list[str]:
    errors: list[str] = []
    if record.get("schema") != "stegverse.fi-transition-continuity-interop.canonical-execution-evidence.v1":
        errors.append("canonical.schema mismatch")
    if record.get("suite_id") != "fi-transition-continuity-interoperability-v0.1":
    ...
    if record.get("issue") != 4:
        errors.append("canonical.issue must equal 4")
    ...
    if not isinstance(commands, list) or len(commands) != 3:
        errors.append("canonical.commands must contain exactly three commands")

    task_results = record.get("task_results")
    if not isinstance(task_results, dict) or set(task_results) != REQUIRED_TASKS:
        errors.append("canonical.task_results must contain exactly all three FI tasks")
    ...
    if not isinstance(hashes, dict) or set(hashes) != REQUIRED_HASHES:
        errors.append("canonical.artifact_hashes must contain exactly three required SHA-256 values")
    else:
    ...
    if not isinstance(equivalence, dict) or set(equivalence) != REQUIRED_EQUIVALENCE:
        errors.append("canonical.artifact_equivalence must contain exactly three required checks")
    else:
...
def main() -> int:
    ...
    result = {
        "schema": "stegverse.fi-transition-continuity-interop.canonical-evidence-gate-result.v1",
        "mode": mode,
